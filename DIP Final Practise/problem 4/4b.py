...
def apply_ideal_low_pass_filter(fft_image, cutoff):
    ...
    # Ideal Low Pass Filter Mask তৈরি
    H = np.zeros_like(fft_image)
    H[D <= cutoff] = 1

    # ফিল্টার প্রয়োগ এবং inverse FFT
    filtered_fft = fft_image * H
    result = np.fft.ifft2(np.fft.ifftshift(filtered_fft))
    return np.abs(result)
...
# -------------------------------
# Apply Ideal Low Pass Filters with different cutoff radius
# -------------------------------
cutoff_values = [5, 10, 20, 30, 40, 50]

plt.figure(figsize=(12, 8))
for i, D0 in enumerate(cutoff_values):
    filtered_img = apply_ideal_low_pass_filter(fft_image, cutoff=D0)
    plt.subplot(2, 3, i+1)
    plt.imshow(filtered_img, cmap='gray')
    plt.title(f'Cutoff = {D0}')
    plt.axis('off')

plt.suptitle("Ideal Low Pass Filter with Different Cutoff Frequencies", fontsize=14)
plt.tight_layout()
plt.show()'''
